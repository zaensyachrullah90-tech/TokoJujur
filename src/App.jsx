...
export default function App() {
  ...
  const handleScanBarcode = async (e) => {
    ...
    if (!('BarcodeDetector' in window)) {
        showToast('Kamera tidak didukung', 'error');
        return;
    }
    try {
  ...
  const handleAddProduct = async (e) => {
    ...
    if (supabase) {
      supabase.from('produk').insert([{
          ...
          if (error) showToast('Gagal Sinkron Barang', 'error');
          else fetchProducts(); 
      });
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    if (filteredForExport.length === 0) {
      showToast('Tidak ada data tanggal ini', 'error');
      return;
    }
  ...
  return (
    <>
      ...
      {supabase && isLoadingDB && (
        <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 text-center">
          <div className="relative mb-6">
            <div className="absolute inset-0 bg-emerald-500 blur-2xl opacity-40 animate-pulse rounded-full w-24 h-24 mx-auto"></div>
            <Store className="animate-bounce relative z-10 text-emerald-400 mx-auto" size={80} />
          </div>
          <h2 className="font-extrabold text-3xl tracking-widest bg-gradient-to-r from-emerald-400 to-teal-200 bg-clip-text text-transparent">TOKO KEJUJURAN</h2>
          <p className="text-slate-400 mt-3 text-sm font-semibold flex items-center gap-2"><Loader2 className="animate-spin" size={16}/> Menyiapkan Barang...</p>
        </div>
      )}
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col">
          ...
          <main className="p-4 flex-1 max-w-md mx-auto w-full">
            ...
            {metodeBayar === 'transfer' && (() => {
               ...
               const anMatch = rekStr.match(/a\.?n\.?\s*(.*)/i);
               const namaPemilik = anMatch ? anMatch[1] : rekStr.replace(noRek, '').replace(bank, '').replace(/[-:]/g, '').trim() || 'Pemilik';
               ...
               return (
                 <div className="mt-6 p-6 bg-white rounded-2xl flex flex-col items-center animate-fade-in border shadow-sm">
                   ...
                   <p className="text-sm text-gray-600 font-medium">{bank} - A.n {namaPemilik}</p>
                 </div>
               );
            })()}
          </main>
      ...
      {supabase && !isLoadingDB && view === 'struk' && strukTerakhir && (
