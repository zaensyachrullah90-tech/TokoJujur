...
const getDynamicIcon = (namaBarang) => {
  ...
  const iconWrapper = (emoji) => (
    <span className="text-5xl md:text-6xl drop-shadow-md transition-transform transform hover:scale-110 will-change-transform">
      {emoji}
    </span>
  );
...
function MainApp() {
  ...
  // =========================================================================
  // RENDER HELPERS (MEMECAH KOMPONEN AGAR BEBAS DARI SYNTAX ERROR)
  // =========================================================================
                    ...
                    <table className="w-full text-left min-w-[600px]">
                       <thead className="bg-slate-50 sticky top-0 shadow-sm z-10">
                         <tr className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                           ...
                           <th className="p-3 md:p-4 text-center">Total Awal</th>
                ...
                {showAddForm && (
                  <form onSubmit={handleAddProduct} className={`p-5 md:p-8 rounded-3xl md:rounded-[40px] border shadow-sm mb-8 grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 animate-slide-up ${editingId ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-200'}`}>
                     <div className="md:col-span-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 border-b border-slate-200 pb-3 mb-1">
                       <h3 className="font-black text-slate-800 text-lg flex items-center gap-2">{editingId ? <><Edit className="text-blue-600" size={20}/> Edit Data Barang</> : <><PlusCircle className="text-emerald-600" size={20}/> Input Barang Baru</>}</h3>
                     </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-emerald-100 relative">
          ...
          <header className="bg-white p-4 shadow-sm sticky top-0 z-40 mb-4 border-b">
            <div className="flex justify-between items-center mb-4 max-w-5xl mx-auto">
              ...
              <div className="flex items-center gap-1.5 md:gap-2 shrink-0">
                ...
                <button onClick={handleExitApp} className="p-2 md:p-2.5 bg-rose-50 text-rose-500 rounded-full hover:bg-rose-100 transition shadow-sm" title="Keluar Aplikasi"><Power size={18}/></button>
              </div>
            </div>
      ...
      {view === 'admin' && isAdminLogged && renderAdminDashboard()}
