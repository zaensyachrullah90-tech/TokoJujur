...
function MainApp() {
        ...
        if (prodRes.data) {
           ...
           setProducts(prev => {
              ...
              return prodRes.data.map(newP => {
                 const oldP = prev.find(op => op.id === newP.id);
                 return oldP && oldP.stok_real !== undefined ? { ...newP, stok_real: oldP.stok_real } : newP;
              });
           });
        }
  ...
  const handleEnhanceWithAI = async () => {
    ...
    setIsProcessing(true);
    showToast('Gemini AI sedang membersihkan latar belakang foto...', 'success');
    try {
      ...
      if (!res.ok) {
         showToast(`Error Gemini: ${data.error?.message || 'Gagal'}`, 'error');
         setIsProcessing(false);
         return;
      }
      ...
      if (outputBase64) {
        ...
        showToast('Gagal merapikan. Format AI tidak didukung atau limit habis.', 'error');
      }
    } catch (err) { 
      showToast('Koneksi AI Gagal/Timeout. Cek kuota API Key.', 'error'); 
    }
    setIsProcessing(false);
  };
